(function() {
  ...
  const TEXT_REGEX = /\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)[a-z]*\.?\s+(\d{1,2}),?\s+(\d{4})\b/gi;
  ...
  function getBrowserDateFormat() {
    const parts = new Intl.DateTimeFormat().formatToParts(new Date(2025, 11, 25));
    const format = parts.findIndex(p => p.type === 'day') > parts.findIndex(p => p.type === 'month') ? 'US' : 'AU';
  ...
  function checkUrlChange() {
    ...
    const check = (list) => list.some(p => p instanceof RegExp ? p.test(currentUrl) : currentUrl.includes(p));

    let newFormat = null;
    if (typeof US_URLS !== 'undefined' && check(US_URLS)) newFormat = 'US';
    else if (typeof AU_URLS !== 'undefined' && check(AU_URLS)) newFormat = 'AU';
    else if (typeof AUTO_URLS !== 'undefined' && check(AUTO_URLS)) newFormat = getBrowserDateFormat();

    if (newFormat !== currentSourceFormat) {
      console.log(`[DateFixer] Format context switched: ${currentSourceFormat} -> ${newFormat}`);
      currentSourceFormat = newFormat;

      if (currentSourceFormat) {
        console.log(`[DateFixer] Initializing processing for ${currentSourceFormat} format...`);
        processNode(document.body);
        startObserver();
      } else {
        if (observer) {
          console.log("[DateFixer] No match for this URL. Disconnecting observer.");
          observer.disconnect();
        }
      }
    } else if (currentSourceFormat) {
      console.debug("[DateFixer] URL changed but format remains same. Re-scanning new page content.");
      processNode(document.body);
    }
  }
  ...
  function formatText(text) {
    ...
    let processed = text.replace(NUMERIC_REGEX, (match, p1, p2, p3) => {
      matchFound = true;
      const year = p3;
      const month = currentSourceFormat === 'US' ? p1.padStart(2, '0') : p2.padStart(2, '0');
      const day   = currentSourceFormat === 'US' ? p2.padStart(2, '0') : p1.padStart(2, '0');
      return `${year}-${month}-${day}`;
    });

    // B. Handle Text Dates (Dec 19, 2025)
    processed = processed.replace(TEXT_REGEX, (match, monthName, day, year) => {
      matchFound = true;
      const monthNum = MONTH_MAP[monthName.toLowerCase().substring(0, 3)];
      return `${year}-${monthNum}-${day.padStart(2, '0')}`;
    });

    if (matchFound) console.debug(`[DateFixer] Transformed: "${text.trim()}" -> "${processed.trim()}"`);
    return processed;
  }
    ...
    if (node.nodeType === 3) {
      ...
      for (const div of targets) {
        const walker = document.createTreeWalker(div, NodeFilter.SHOW_TEXT, {
          ...
          acceptNode: (n) => ['SCRIPT', 'STYLE', 'CODE', 'SMALL', 'TEXTAREA'].includes(n.parentElement?.tagName)
  ...
  function debouncedProcess() {
    ...
    console.debug(`[DateFixer] Debounced batch complete. Processed ${nodeCount} changed nodes.`);
    observe();
  }
  ...
  function init() {
    console.log("[DateFixer] Content script injected. Waiting for URL match...");
